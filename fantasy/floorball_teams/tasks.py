...
@shared_task
def get_teams():
    request = requests.get(f'{Enum.BASE_URL}{Enum.TEAMS_URL}')
    soup = BeautifulSoup(request.content, 'html.parser')
    ...
    for row in rows:
        ...
        team_url = f'{Enum.BASE_URL}{cells[2].find("a")["href"]}'
        wins = cells[4].text
        ...
        try:
            ...
            team = FloorballTeam(
                ...
                points=points, image_url=f'{Enum.BASE_URL}{get_team_img_url(team_url)}', team_url=team_url
            )
            team.save()
    return 'DONE'
